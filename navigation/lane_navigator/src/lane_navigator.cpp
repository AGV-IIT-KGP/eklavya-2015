...
int main(int argc, char **argv) {
    ...
    while(ros::ok()){
    node_handle.getParam(node_name + "/debug", local_to_odom.debug);
    pose2d = local_to_odom.get_odom_target();
    geometry_msgs::PoseStamped pose_stamped;
    pose_stamped = local_to_odom.convert_Pose2D_to_PoseStamped(pose2d);
    local_to_odom.pub_point.publish(pose_stamped);
    ros::spinOnce();
    }
    return 0;
}
